...
EXP=${EXP:-picf_a7_object_candidate_anchor1000_20260519}
...
echo "[run] verifying object-candidate slot-binding contract"
...
"${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node=2 scripts/picf_core_train.py \
  ...
  --anchor-pv-object-distribution-confirmation-threshold 0.05 \
  ...
  --tracklet-read-weight 0.05 \
  --tracklet-confidence-floor 0.10 \
