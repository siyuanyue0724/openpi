...
if [[ "$LOAD_GLOBAL_STEP" == 0 ]]; then
  ...
  [[ "$RUN_DIR" == /mnt/* && ! -e "$RUN_DIR" && ! -L "$RUN_DIR" ]] || {
    echo "fresh ADR-207 run directory must be absent beneath /mnt" >&2
    exit 2
  }
else
  [[ "$LOAD_GLOBAL_STEP" =~ ^[0-9]+$ ]] \
    && (( LOAD_GLOBAL_STEP > 0 && LOAD_GLOBAL_STEP % 2000 == 0 )) || {
    echo "ADR-207 resume requires a positive 2k checkpoint boundary" >&2
    exit 2
  }
  export PICF_PHASE=resume
  export PICF_LOAD_GLOBAL_STEP=$LOAD_GLOBAL_STEP
  [[ "$RUN_DIR" == /mnt/* && -d "$RUN_DIR" && ! -L "$RUN_DIR" ]] || {
    echo "resume ADR-207 run directory must be an existing direct path beneath /mnt" >&2
    exit 2
  }
fi

EXPECTED_REPO=${PICF_ADR207_REPO:-/mnt/picf-next/adr207/source-freezes/native-query-posterior-v18}
[[ -d "$EXPECTED_REPO" && ! -L "$EXPECTED_REPO" ]] || {
  echo "ADR-207 immutable source freeze is absent: $EXPECTED_REPO" >&2
  exit 1
}
[[ -f "$EXPECTED_REPO/source-freeze.receipt.json" && ! -L "$EXPECTED_REPO/source-freeze.receipt.json" ]] || {
  echo "ADR-207 immutable source freeze receipt is absent" >&2
  exit 1
}
[[ -z "$(git -C "$EXPECTED_REPO" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "ADR-207 candidate requires the exact clean source freeze" >&2
  exit 1
}
...
export PICF_CONTRACT_ROOT=${PICF_ADR207_CONTRACT_ROOT:-/mnt/picf-next/adr207/contracts/native-query-posterior-${WORLD_SIZE}gpu-30k-v1}
export PICF_ADR178_CACHE_ROOT=${PICF_ADR207_CACHE_ROOT:-/mnt/picf-next/adr207/caches/native-query-posterior-${WORLD_SIZE}gpu-30k-v1}
case "${PICF_ADR210_ENABLE_DENSE_SUPPLEMENT:-0}" in
  0)
    export PICF_USE_DENSE_SUPPLEMENT=0
    ;;
  1)
    export PICF_USE_DENSE_SUPPLEMENT=1
    ;;
  *)
    echo "PICF_ADR210_ENABLE_DENSE_SUPPLEMENT must be 0 or 1" >&2
    exit 2
    ;;
esac
...
[[ -f "$RUNTIME_OVERLAY_RECEIPT" && ! -L "$RUNTIME_OVERLAY_RECEIPT" ]] || {
  echo "ADR-207 runtime overlay receipt is absent" >&2
  exit 1
}
[[ "$(sha256sum "$RUNTIME_OVERLAY_RECEIPT" | cut -d' ' -f1)" == "$RUNTIME_OVERLAY_RECEIPT_SHA256" ]] || {
  echo "ADR-207 runtime overlay receipt changed" >&2
  exit 1
}
...
[[ -f "$BASE" && ! -L "$BASE" ]] || {
  echo "ADR-207 base launcher is absent from its immutable source freeze" >&2
  exit 1
}
exec "$BASE" "$RUN_DIR"
