...
[[ "$CONTRACT_ROOT" == /mnt/* && ! -e "$CONTRACT_ROOT" && ! -L "$CONTRACT_ROOT" ]] || {
  echo "ADR-207 contract root must be one absent persistent path beneath /mnt" >&2
  exit 2
}
...
REFERENCE_SPLIT=${PICF_ADR207_REFERENCE_SPLIT:-/mnt/picf-next/adr177/contracts/full-modal-2gpu-30k-v1/representation-split.json}
...
for path in "$PYTHON" "$REPO/source-freeze.receipt.json" "$DATASET_MANIFEST" "$REFERENCE_SPLIT"; do
  [[ -f "$path" && ! -L "$path" ]] || {
...
[[ -z "$(git -C "$REPO" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "ADR-207 contract construction requires the exact clean source freeze" >&2
  exit 1
}
...
mkdir -p "$PARENT"
STAGING=$(mktemp -d "$PARENT/.native-query-posterior-${WORLD_SIZE}gpu-30k-v1.XXXXXX")
cleanup() {
  rm -rf "$STAGING"
}
trap cleanup EXIT
...
"$PYTHON" - "$STAGING" "$REFERENCE_SPLIT" "$WORLD_SIZE" <<'PY'
import hashlib
import json
import os
...
payload = {
    "schema": "picf-next.adr207-contract-freeze/v1",
    "comparison_id": "adr207-native-videomt-query-posterior-v1",
    "future_source_frame_count": 4,
