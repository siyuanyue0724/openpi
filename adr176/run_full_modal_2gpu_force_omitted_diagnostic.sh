...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 FORCED_STEP RUN_DIR" >&2
  exit 2
fi

FORCED_STEP=$1
RUN_DIR=$2
[[ "$FORCED_STEP" =~ ^([1-9]|[12][0-9]|3[0-2])$ ]] || {
  echo "FORCED_STEP must be an integer in [1, 32]" >&2
  exit 2
}
...
# Engineering-only reproduction of the first omitted-static update.  This
# deliberately forces the already registered omitted-static branch on one
# selected step while the model, data, caches, loss graph, and FSDP2 placement
# remain identical to training. PICF_DIAGNOSTIC_STOP_AFTER_STEP may retain a
# few following ordinary steps to test whether the omitted transaction leaves
# the next FSDP2 backward in a valid state.
# It is not a scientific arm and remains a bounded engineering reproduction.
...
BACKWARD_PREFETCH=${PICF_FSDP2_BACKWARD_PREFETCH:-default}
FACTUAL_GRADIENT_STORAGE=${PICF_SEQUENTIAL_FACTUAL_GRADIENT_STORAGE:-gpu}
FSDP2_PLACEMENT=${PICF_FSDP2_PLACEMENT:-selective-embedding-offload}
OMITTED_REMATERIALIZATION=${PICF_OMITTED_STATIC_REMATERIALIZATION:-sequential-backward}
CUDA_ALLOCATOR=${PICF_CUDA_ALLOCATOR:-native}
STOP_AFTER_STEP=${PICF_DIAGNOSTIC_STOP_AFTER_STEP:-$FORCED_STEP}
...
[[ "$STOP_AFTER_STEP" =~ ^([1-9]|[12][0-9]|3[0-2])$ ]] || {
  echo "PICF_DIAGNOSTIC_STOP_AFTER_STEP must be an integer in [1, 32]" >&2
  exit 2
}
(( STOP_AFTER_STEP >= FORCED_STEP )) || {
  echo "PICF_DIAGNOSTIC_STOP_AFTER_STEP must not precede FORCED_STEP" >&2
  exit 2
}
...
exec "$PYTHON" -m torch.distributed.run \
  ...
  --engineering-force-omitted-static-step "$FORCED_STEP" \
  --seed 20260721 \
  --capacity 16 \
  ...
  --omitted-static-rematerialization "$OMITTED_REMATERIALIZATION" \
