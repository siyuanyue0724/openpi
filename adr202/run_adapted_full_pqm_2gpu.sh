...
EXPECTED_REPO=/mnt/picf-next/adr202/source-freezes/adapted-full-pqm-v2
...
if [[ -n ${PICF_REPO:-} && "$PICF_REPO" != "$EXPECTED_REPO" ]]; then
  echo "ADR-202 forbids a non-frozen PICF_REPO override: $PICF_REPO" >&2
  exit 2
fi
if [[ -n ${PICF_LINGBOT_NATIVE_SOURCE:-} && "$PICF_LINGBOT_NATIVE_SOURCE" != "$EXPECTED_SOURCE" ]]; then
  echo "ADR-202 forbids a different prepared LingBot checkout" >&2
  exit 2
fi
...
export PICF_VIDEOMT_STAGE_PQ_MODE=frozen-adapted-eval-causal-c5-pqm
...
REUSED_AUDIT=${PICF_REUSED_FULL_TRANSPLANT_AUDIT:-}
...
if [[ -n "$REUSED_AUDIT" ]]; then
  [[ -f "$REUSED_AUDIT" && ! -L "$REUSED_AUDIT" ]] || {
    echo "reused full-transplant audit is absent or a symlink" >&2
    exit 1
  }
  "$PYTHON" - "$REUSED_AUDIT" "$AUDIT_OUTPUT" <<'PY'
import json
import os
import sys
...
if (
    report.get("schema") != "picf-next.full-transplant-audit.v1"
    or report.get("status") != "passed"
    or report.get("strict_runtime") is not True
    or report.get("external_checks_complete") is not True
    or report.get("manifest_canonical_sha256")
    != "f7259c1f096f38163e564ceaffe30e9ba93a53b111a1e2680ac1b4ea6938af4b"
):
    raise ValueError("reused full-transplant audit does not authenticate these assets")
payload = {
    "schema": "picf-next.reused-full-transplant-audit/v1",
    "status": "passed",
    "reason": "identical immutable external assets; ADR-202 code and adapted state validate separately",
