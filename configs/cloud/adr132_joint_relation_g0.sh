...
RUN_DIR=${RUN_DIR:?RUN_DIR must name one fresh persistent run directory}
LOG=${LOG:?LOG must name one persistent log file}
OUTPUT=${OUTPUT:-$RUN_DIR/behavior_causal_probe.json}
...
exec env -u PYTORCH_CUDA_ALLOC_CONF -u PYTORCH_ALLOC_CONF \
  ...
  --behavior-causal-probe-output "$OUTPUT" \
  >"$LOG" 2>&1
