...
RUN_DIR=${RUN_DIR:?RUN_DIR must be the completed persistent G1 run directory}
LOG=${LOG:?LOG must name one persistent log file}
OUTPUT=${OUTPUT:-$RUN_DIR/behavior_posterior_control_probe.json}
G1_REPORT_SHA256=${G1_REPORT_SHA256:?G1_REPORT_SHA256 must bind the exact step-2 report}
...
if [[ ! -d "$RUN_DIR/checkpoints/global_step_2" ]]; then
  echo "ADR132 G2 requires the completed G1 step-2 checkpoint" >&2
  exit 2
fi

mkdir -p "$(dirname "$LOG")"
echo $$ > "$RUN_DIR/launcher-g2.pid"
...
exec env -u PYTORCH_CUDA_ALLOC_CONF -u PYTORCH_ALLOC_CONF \
  ...
  --behavior-g1-predecessor-report-sha256 "$G1_REPORT_SHA256" \
  --behavior-posterior-control-probe-output "$OUTPUT" \
  --run-dir "$RUN_DIR" \
  --load-global-step 2 \
  --invocation-steps 1 \
