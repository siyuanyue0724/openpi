...
BASE_RUN=${BASE_RUN:-/mnt/picf-next/runs/adr132-j2-factorized-step0-baseline-v5-$STAMP}
BASE_LOG=${BASE_LOG:-/mnt/picf-next/logs/adr132-j2-factorized-step0-baseline-v5-$STAMP.log}
BASELINE=${BASELINE:-/mnt/picf-next/probes/adr132-factorized-step0-baseline-v5-$STAMP.json}
BASELINE_REPORT=${BASELINE_REPORT:-$BASELINE.build_report.json}
J2_RUN=${J2_RUN:-/mnt/picf-next/runs/adr132-factorized-j2-v5-$STAMP}
J2_LOG=${J2_LOG:-/mnt/picf-next/logs/adr132-factorized-j2-v5-$STAMP.log}
STATE=${STATE:-/mnt/picf-next/runs/adr132-j2-factorized-chain-v5-$STAMP.state}
...
for path in "$BASE_RUN" "$BASE_LOG" "$BASELINE" "$BASELINE_REPORT" "$J2_RUN" "$J2_LOG" "$STATE"; do
...
printf 'phase=baseline_running\nstamp=%s\nbase_run=%s\nbase_log=%s\nbaseline=%s\nj2_run=%s\nj2_log=%s\n' \
  "$STAMP" "$BASE_RUN" "$BASE_LOG" "$BASELINE" "$J2_RUN" "$J2_LOG" >"$STATE"
...
RUN_DIR="$BASE_RUN" LOG="$BASE_LOG" PICF_WORKTREE="$WORKTREE" \
  "$WORKTREE/configs/cloud/adr132_joint_relation_j2_baseline.sh"
...
if baseline["sample_count"] != 136:
    raise RuntimeError("J2 baseline sample coverage changed")
...
RUN_DIR="$J2_RUN" LOG="$J2_LOG" PICF_WORKTREE="$WORKTREE" \
  ...
  "$WORKTREE/configs/cloud/adr132_joint_relation_j2.sh"
