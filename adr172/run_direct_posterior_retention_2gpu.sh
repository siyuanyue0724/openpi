...
retention_timeout_seconds=${PICF_ADR172_RETENTION_TIMEOUT_SECONDS:-3600}
...
[[ "$repository_root" == /mnt/* && "$source_checkout" == /mnt/* ]] || {
  echo "ADR172 retention source trees must live under /mnt" >&2
  exit 1
}
...
[[ "$run_root" == /mnt/* && ! -e "$run_root" && ! -L "$run_root" ]] || {
  echo "ADR172 retention output must be one absent direct path under /mnt" >&2
  exit 1
}
[[ -z "$(git -C "$repository_root" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "ADR172 retention requires an exact clean PICF checkout" >&2
  exit 1
}
[[ "$retention_timeout_seconds" =~ ^[1-9][0-9]*$ ]] || {
  echo "PICF_ADR172_RETENTION_TIMEOUT_SECONDS must be a positive integer" >&2
  exit 1
}
...
for path in "${required[@]}"; do
  [[ -e "$path" ]] || {
    echo "required ADR172 retention artifact is absent: $path" >&2
    exit 1
  }
done
...
exec timeout --signal=TERM --kill-after=60s "$retention_timeout_seconds" \
  ...
  --output "$run_root/adr172_direct_posterior_retention_full_report.json" \
  --mode gate \
  --phase retention \
