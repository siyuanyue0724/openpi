...
[[ "$repository_root" == /mnt/* && "$source_checkout" == /mnt/* ]] || {
  echo "mediator trial source trees must live under /mnt" >&2
  exit 1
}
[[ "$run_root" == /mnt/* && ! -e "$run_root" && ! -L "$run_root" ]] || {
  echo "mediator trial output must be one absent direct path under /mnt" >&2
  exit 1
}
[[ -z "$(git -C "$repository_root" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "mediator trial requires an exact clean PICF checkout" >&2
  exit 1
}
...
for path in "${required[@]}"; do
  [[ -e "$path" ]] || {
    echo "required mediator-trial artifact is absent: $path" >&2
    exit 1
  }
done
...
exec "$python_bin" -m torch.distributed.run --standalone --nproc_per_node=2 \
  ...
  --output "$run_root/ltop_g3_mediator_trial_training_report.json" \
