...
evaluation_report=$evaluation_root/ltop_g3_mediator_cold_action_${evaluation_scope}_report.json

if [[ "$repository_root" != /mnt/* || "$training_root" != /mnt/* ]]; then
  echo "ADR165 action source and training evidence must live under /mnt" >&2
  exit 1
fi
if [[ "$evaluation_root" != /mnt/* || -e "$evaluation_root" || -L "$evaluation_root" ]]; then
  echo "ADR165 action output must be one absent direct path under /mnt" >&2
  exit 1
fi
...
for path in \
  ...
  if [[ ! -e "$path" ]]; then
    echo "required ADR165 action artifact is absent: $path" >&2
    exit 1
  fi
done
if [[ $(git -C "$repository_root" status --porcelain=v1 --untracked-files=all) ]]; then
  echo "ADR165 action evaluation requires an exact clean source checkout" >&2
  exit 1
fi
if [[ $(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l) -ne 2 ]]; then
  echo "ADR165 action evaluation requires exactly two visible GPUs" >&2
  exit 1
fi
...
if [[ "$evaluation_status" -ne 0 ]]; then
  printf '{"schema":"picf-next.adr165-cold-action-runtime-failure.v1","status":"FAIL","exit_code":%d,"timeout_seconds":%d,"evaluation_scenes_per_partition":%d}\n' \
