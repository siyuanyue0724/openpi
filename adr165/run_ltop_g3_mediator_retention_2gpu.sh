...
training_report=$training_root/ltop_g3_mediator_trial_training_report.json
retention_report=$retention_root/ltop_g3_mediator_representation_retention_report.json

if [[ "$repository_root" != /mnt/* || "$training_root" != /mnt/* ]]; then
  echo "ADR165 retention source and training evidence must live under /mnt" >&2
  exit 1
fi
if [[ "$retention_root" != /mnt/* || -e "$retention_root" || -L "$retention_root" ]]; then
  echo "ADR165 retention output must be one absent direct path under /mnt" >&2
  exit 1
fi
...
for path in \
  ...
  "$training_report" \
  "$source_checkout" \
  "$repository_root/adr165/mediator_trial_cold_evaluation_contract.sh" \
  ...
  if [[ ! -e "$path" ]]; then
    echo "required ADR165 retention artifact is absent: $path" >&2
    exit 1
  fi
done
if [[ $(git -C "$repository_root" status --porcelain=v1 --untracked-files=all) ]]; then
  echo "ADR165 retention requires an exact clean source checkout" >&2
  exit 1
fi
if [[ $(nvidia-smi --query-gpu=index --format=csv,noheader | wc -l) -ne 2 ]]; then
  echo "ADR165 retention requires exactly two visible GPUs" >&2
  exit 1
fi

# shellcheck source=adr165/mediator_trial_cold_evaluation_contract.sh
source "$repository_root/adr165/mediator_trial_cold_evaluation_contract.sh"
trained_checkpoint=$(adr165_resolve_mediator_trial_checkpoint "$python_bin" "$training_report")
...
if [[ "$retention_status" -ne 0 ]]; then
  printf '{"schema":"picf-next.adr165-retention-runtime-failure.v1","status":"FAIL","exit_code":%d,"timeout_seconds":%d}\n' \
...
adr165_validate_cold_report \
  "$python_bin" "$retention_report" "$trained_checkpoint" retention 4
