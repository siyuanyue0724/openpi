...
run_one() {
  ...
  local prefix_stopgrad="$9"
  shift 9
  local extra=("$@")
  local log="${LOG_BASE}/${exp}.log"
  local prefix_args=()
  if [[ "${prefix_stopgrad}" == "yes" ]]; then
    prefix_args=(--picf-action-prefix-stopgrad)
  fi
  {
    echo "[active-slot-matrix] exp=${exp} start $(date -Is)"
    echo "[active-slot-matrix] repo=$(pwd) commit=$(git rev-parse --short HEAD)"
    echo "[active-slot-matrix] scope=${scope} steps=${steps} unroll=${unroll} burnin=${burnin} action=${action_weight} active_max=${active_max} overlap=${active_overlap} prefix_stopgrad=${prefix_stopgrad}"
    ...
    echo "[active-slot-matrix] exp=${exp} done $(date -Is)"
  } 2>&1 | tee -a "${log}"
}

case "${PROFILE}" in
  a5)
    run_one picf_a5_activecap_anchor_u1b4_a025_600_ac273a2 600 anchor_only 1 4 0.25 4 0.75 no
    run_one picf_a5_activecap_anchor_u2b1_a025_600_ac273a2 600 anchor_only 2 1 0.25 4 0.75 no
    run_one picf_a5_activecap_anchor_u2b1_max6_a025_600_ac273a2 600 anchor_only 2 1 0.25 6 0.70 no
    ;;
  a7)
    run_one picf_a7_activecap_cotrain_prefix_u2b1_a1_600_ac273a2 600 all 2 1 1.0 4 0.75 yes
    run_one picf_a7_activecap_cotrain_flow_u2b1_a025_450_ac273a2 450 all 2 1 0.25 4 0.75 no
    run_one picf_a7_activecap_cotrain_prefix_u2b1_max6_a1_600_ac273a2 600 all 2 1 1.0 6 0.70 yes
    ;;
  *)
