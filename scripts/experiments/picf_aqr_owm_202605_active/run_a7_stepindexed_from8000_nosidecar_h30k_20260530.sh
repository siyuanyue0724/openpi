...
# Corrected-horizon sidecar/noise ablation from the preserved step8000 basin.
#
# Purpose:
#   Test whether contact-motion sidecar proposal/tracklet evidence is the
#   dominant action-platform noise source.
#
# Critical comparability rule:
#   NUM_TRAIN_STEPS stays at 30000 even though this is normally stopped after
#   100-300 diagnostic steps.  The global step count is part of the LR schedule;
#   using 8300 would push the scheduler near min_lr at step8000 and confound the
#   sidecar ablation with a low-LR ablation.
#
# Mechanism:
#   SEGMENTS are read from the clean full sidecar root so the sampled CALVIN
#   segment set is identical to the production run.  Only optional
#   tracklet/proposal arrays disappear by pointing mvtrack_sidecar_root at an
...
export EXP="${EXP:-picf_a7_stepindexed_from8000_nosidecar_h30k_action2_20260530}"
