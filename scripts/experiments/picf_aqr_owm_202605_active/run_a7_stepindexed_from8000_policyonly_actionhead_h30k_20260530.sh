...
# Frozen-PICF + frozen-PaliGemma action-head transfer probe.
#
# Purpose:
#   If E2 (frozen PICF, train PaliGemma+action) matches the reset baseline,
#   this sharper split checks whether the action plateau is explainable by the
#   action/policy head alone.  It keeps the same step8000 prefix, full sidecar,
#   step-indexed RNG, and h30k schedule, but freezes semantic/PICF structure.
...
export EXP="${EXP:-picf_a7_stepindexed_from8000_policyonly_actionhead_h30k_20260530}"
...
# The action-head-only split leaves only four small wrapper-local float32
# modules trainable while the frozen PaliGemma/Gemma stack remains bf16.  FSDP
# root flattening rejects that mixed dtype set, so this causal probe uses DDP.
# A100 40GB has enough memory because only the wrapper-local heads receive grads.
...
# Keep the semantic wrapper trainable so its internal action/time heads can be
# selected, but freeze the PaliGemma/Gemma backbone/expert through the
# action_head_only scope.  Setting SEMANTIC_TRAINABLE=0 would leave no non-core
# trainable parameters under picf_trainable_scope=policy_only.
export SEMANTIC_TRAINABLE="${SEMANTIC_TRAINABLE:-1}"
export SEMANTIC_TRAINABLE_SCOPE="${SEMANTIC_TRAINABLE_SCOPE:-action_head_only}"
export SEMANTIC_GRADIENT_CHECKPOINTING="${SEMANTIC_GRADIENT_CHECKPOINTING:-0}"
