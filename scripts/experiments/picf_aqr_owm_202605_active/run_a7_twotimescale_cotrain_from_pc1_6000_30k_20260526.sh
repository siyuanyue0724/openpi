...
# Two-timescale cotrain validation.
#
# Purpose:
#   Keep the production cotrain principle while addressing the May-24/25
#   action rebound root cause.  PaliGemma/action remains trainable at normal
#   action-learning speed; PICF `core.*` remains trainable but moves on a
#   slower optimizer timescale so the action-visible prefix is not a fast
...
#   - PaliGemma/action path trainable;
#   - PICF core trainable with low LR scale;
#   - object scaffold weak and decayed;
#   - unsafe raw predictive losses disabled by the shared base launcher.
...
export EXP="${EXP:-picf_a7_twotime_cotrain_from_pc1_6000_core005_action2_30k_20260526}"
...
export PICF_CORE_LR_SCALE="${PICF_CORE_LR_SCALE:-0.05}"
