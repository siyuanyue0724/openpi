...
# Low-LR fresh-optimizer current-phase probe from A7 step4500.
#
# The original strict step2500 LR control was invalidated by checkpoint
# retention.  This run instead asks whether the current A7 plateau state can be
# released by a low-LR fresh optimizer while preserving A7's cleaner structure.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export REPO_ROOT="${REPO_ROOT:-/root/openpi_2500fresh_20260525}"
export PYTHON_BIN="${PYTHON_BIN:-/root/openpi/.venv/bin/python}"
export EXP="${EXP:-picf_pc1_from_a7_4500_freshopt_lr2e5_30k_20260525}"
export RESUME_CHECKPOINT="${RESUME_CHECKPOINT:-/mnt/checkpoints/picf_core/picf_core/picf_a7_from_a5_1500_freshopt_actionpolish_30k_20260524/4500}"
