...
OUTPUT_ROOT=${PICF_ADR209_DENSE_CACHE_ROOT:-/mnt/picf-next/adr209/caches/flare-4gpu-prefix${TRAINING_PREFIX_STEPS}-v1}
...
if [[ -e "$OUTPUT.receipt.json" || -L "$OUTPUT.receipt.json" ]]; then
  echo "completed ADR-209 cache receipt already exists: $OUTPUT.receipt.json" >&2
  exit 2
fi
