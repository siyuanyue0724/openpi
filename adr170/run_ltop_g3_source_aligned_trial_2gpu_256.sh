...
trial_timeout_seconds=${PICF_G3_SOURCE_TRIAL_TIMEOUT_SECONDS:-10800}

[[ "$repository_root" == /mnt/* && "$source_checkout" == /mnt/* ]] || {
  echo "source-aligned trial source trees must live under /mnt" >&2
  exit 1
}
[[ "$run_root" == /mnt/* && ! -e "$run_root" && ! -L "$run_root" ]] || {
  echo "source-aligned trial output must be one absent direct path under /mnt" >&2
  exit 1
}
[[ -z "$(git -C "$repository_root" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "source-aligned trial requires an exact clean PICF checkout" >&2
  exit 1
}
[[ "$trial_timeout_seconds" =~ ^[1-9][0-9]*$ ]] || {
  echo "PICF_G3_SOURCE_TRIAL_TIMEOUT_SECONDS must be a positive integer" >&2
  exit 1
}
...
for path in "${required[@]}"; do
  [[ -e "$path" ]] || {
    echo "required source-aligned trial artifact is absent: $path" >&2
    exit 1
  }
done
...
exec timeout --signal=TERM --kill-after=60s "$trial_timeout_seconds" \
  ...
  --output "$run_root/ltop_g3_source_aligned_trial_training_report.json" \
  --progress-output "$run_root/progress.json" \
  --journal-dir "$run_root/rank_journal" \
  --checkpoint-output "$run_root/checkpoint-model-only" \
  --mode mediator-trial \
  --phase training \
  --steps 256 \
  --eval-every 32 \
