...
EXPECTED_REPO=/mnt/picf-next/adr204/source-freezes/released-query-propagation-v7-block-checkpoint
...
if [[ -n ${PICF_REPO:-} && "$PICF_REPO" != "$EXPECTED_REPO" ]]; then
  echo "ADR-205 forbids a non-frozen PICF_REPO override: $PICF_REPO" >&2
  exit 2
fi
if [[ -n ${PICF_LINGBOT_NATIVE_SOURCE:-} && "$PICF_LINGBOT_NATIVE_SOURCE" != "$EXPECTED_SOURCE" ]]; then
  echo "ADR-205 forbids a different prepared LingBot checkout" >&2
  exit 2
fi

export PICF_ARCHITECTURE_PROFILE=adr205_released_query_propagation_v1
