...
EXPECTED_REPO=/mnt/picf-next/adr204/source-freezes/full-source-row-refinement-v5
...
if [[ -n ${PICF_REPO:-} && "$PICF_REPO" != "$EXPECTED_REPO" ]]; then
  echo "ADR-204 final-only forbids a non-frozen PICF_REPO override: $PICF_REPO" >&2
  exit 2
fi
if [[ -n ${PICF_LINGBOT_NATIVE_SOURCE:-} && "$PICF_LINGBOT_NATIVE_SOURCE" != "$EXPECTED_SOURCE" ]]; then
  echo "ADR-204 final-only forbids a different prepared LingBot checkout" >&2
  exit 2
fi

export PICF_ARCHITECTURE_PROFILE=adr204_full_source_final_only_v1
