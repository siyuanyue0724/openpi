...
- transformer stacks on this profile now clone incoming batched view aliases once at stack entry before attention. PICF builds many `[1, T, C]` batches via `tokens[None, :]`; the clone is mathematically exact and avoids FSDP/autograd multi-view alias failures inside residual attention blocks
