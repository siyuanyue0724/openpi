...
REPO=${PICF_REPO:-/mnt/picf-next/source-checkouts/picf-next-fe25fa047837-v1}
...
[[ ${#GPU_ROWS[@]} -eq 2 ]] || {
  echo "two-GPU runtime restore requires exactly two visible GPUs" >&2
  exit 1
}
for row in "${GPU_ROWS[@]}"; do
  [[ "$row" == *"A100"* && "$row" == *"40960 MiB"* ]] || {
    echo "two-GPU runtime restore requires A100 40GB; observed: $row" >&2
    exit 1
  }
done

[[ -f "$ARCHIVE" && -f "$ARCHIVE.sha256" ]] || {
  echo "audited runtime archive or checksum is absent" >&2
  exit 1
}
...
# Recreate the environment from accepted bytes on every cold instance. Package
# metadata alone cannot detect mutable site-packages inherited from an old VM.
...
PICF_LINGBOT_NATIVE_SOURCE="$SOURCE" \
...
if torch.cuda.device_count() != 2:
    raise RuntimeError(f"unexpected CUDA device count: {torch.cuda.device_count()}")
...
print(
    "runtime=PASS "
    f"policy={LingbotVlaV2Policy.__name__} "
    f"torch={torch.__version__} "
    f"cuda_devices={torch.cuda.device_count()}"
)
PY

echo "two-GPU runtime preflight=PASS"
