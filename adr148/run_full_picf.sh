...
[[ "$RUN_DIR" == /mnt/* ]] || {
  echo "ADR-148 run directory must be persistent under /mnt" >&2
  exit 2
}
...
case "$PHASE" in
  fresh)
    [[ "$LOAD_GLOBAL_STEP" -eq 0 && "$STOP_AFTER_STEP" -le 2000 && ! -e "$RUN_DIR" ]] || {
      echo "fresh ADR-148 requires step zero, an absent run directory, and a gate at or before 2000" >&2
      exit 2
    }
    ;;
  resume)
    [[ "$LOAD_GLOBAL_STEP" -gt 0 && $((LOAD_GLOBAL_STEP % 2000)) -eq 0 ]] || {
      echo "ADR-148 resume requires a positive 2000-step boundary" >&2
      exit 2
    }
    [[ -d "$RUN_DIR/checkpoints/global_step_$LOAD_GLOBAL_STEP" ]] || {
      echo "ADR-148 resume checkpoint is absent" >&2
      exit 1
    }
    ;;
  *)
...
CURRENT_CACHE=${PICF_CURRENT_CACHE_ROOT:-/mnt/picf-next/adr148/full-picf-30k/cache/current-correction-dino-v1}
...
[[ -z "$(git -C "$REPO" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "ADR-148 execution requires an exact clean checkout" >&2
  exit 1
}

mapfile -t GPU_ROWS < <(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader)
[[ ${#GPU_ROWS[@]} -eq 4 ]] || {
  echo "ADR-148 requires exactly four visible GPUs" >&2
  exit 1
}
for row in "${GPU_ROWS[@]}"; do
  [[ "$row" == *"A100"* && "$row" == *"40960 MiB"* ]] || {
    echo "ADR-148 requires four A100 40GB GPUs; observed: $row" >&2
    exit 1
  }
done
