...
EXPECTED_REPO=/mnt/picf-next/adr203/source-freezes/source-faithful-row-mask-v2
...
if [[ -n ${PICF_REPO:-} && "$PICF_REPO" != "$EXPECTED_REPO" ]]; then
  echo "ADR-203 forbids a non-frozen PICF_REPO override: $PICF_REPO" >&2
  exit 2
fi
if [[ -n ${PICF_LINGBOT_NATIVE_SOURCE:-} && "$PICF_LINGBOT_NATIVE_SOURCE" != "$EXPECTED_SOURCE" ]]; then
  echo "ADR-203 forbids a different prepared LingBot checkout" >&2
  exit 2
fi

export PICF_ARCHITECTURE_PROFILE=adr193_implicit_multimodal_anchor_v1
export PICF_STOP_AFTER_STEP=${PICF_STOP_AFTER_STEP:-100}
export PICF_REPO=$EXPECTED_REPO
export PICF_LINGBOT_NATIVE_SOURCE=$EXPECTED_SOURCE
export PICF_VIDEOMT_STAGE_PQ_MODE=frozen-adapted-eval-causal-c5-pqmr
