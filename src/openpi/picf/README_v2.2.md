...
- safe core stacks are now explicit FSDP child boundaries on this profile: `token_fusion`, `obs_self`, `posterior_self`, `task_self`, `predictive_world`, `predictive_semantic_world`, and `control_world`; root FSDP therefore only carries the remaining lighter core/projection parameters instead of one monolithic core shard
- transformer-stack entry now breaks incoming batched view aliases once (`x = x.clone()` when the input has a base view) before attention. This is mathematically exact and is now part of the 4x40GB contract because many PICF call sites batch tokens via `tokens[None, :]`; under FSDP full-shard those aliasing views can otherwise trip autograd's multi-view in-place checks inside residual attention blocks
