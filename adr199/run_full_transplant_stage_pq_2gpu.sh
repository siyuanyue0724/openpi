...
EXPECTED_REPO=/mnt/picf-next/adr199/source-freezes/full-transplant-stage-pq-f6e0740a-v8
...
if [[ -n ${PICF_REPO:-} && "$PICF_REPO" != "$EXPECTED_REPO" ]]; then
  echo "ADR-199 forbids a non-v8 PICF_REPO override: $PICF_REPO" >&2
  exit 2
fi
if [[ -n ${PICF_LINGBOT_NATIVE_SOURCE:-} && "$PICF_LINGBOT_NATIVE_SOURCE" != "$EXPECTED_SOURCE" ]]; then
  echo "ADR-199 forbids a different prepared LingBot checkout" >&2
  exit 2
fi
...
export PICF_VIDEOMT_STAGE_PQ_MODE=frozen-released-eval-causal-c5
export PICF_VIDEOMT_IDLE_PLACEMENT=cpu-between-forwards
